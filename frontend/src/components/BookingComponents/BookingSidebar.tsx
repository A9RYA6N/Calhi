    ...
    return (
        <div className="w-full md:w-1/3 bg-[#0a0a0a]/80 border-b md:border-b-0 md:border-r border-white/5 p-8 flex flex-col gap-6 relative">
            ...
            {step === 2 && (
                <button
                    onClick={() => setStep(1)}
                    className="w-10 h-10 rounded-full bg-[#171717] border border-white/5 flex items-center justify-center text-[#9ca3af] transition-all duration-300 mb-10 group relative z-10"
                >
                    <span className="material-symbols-outlined text-xl group-hover:-translate-x-1 transition-transform">arrow_back</span>
                </button>
            )}
            {step === 1 && (
                <Link to={`/${username}`} className="w-10 h-10 rounded-full bg-[#171717] border border-white/5 flex items-center justify-center text-[#9ca3af] transition-all duration-300 mb-10 group relative z-10">
                    <span className="material-symbols-outlined text-xl group-hover:-translate-x-1 transition-transform">arrow_back</span>
                </Link>
            )}
            ...
            <div className="mt-2 relative z-10">
                <h1 className="font-headline font-black text-4xl md:text-5xl text-white tracking-tight leading-tight mb-6">
                    {timeslot?.EventName || "Loading..."}
                </h1>
                <div className="flex flex-col gap-4 text-[#9ca3af]">
                    <div className="flex items-center gap-3">
                        <span className="material-symbols-outlined text-[#9ca3af] mt-0.5">schedule</span>
                        <span className="font-body text-base text-white font-medium">{timeslot ? formatDuration(timeslot.Duration) : ""}</span>
                    </div>
                    <div className="flex items-start gap-3">
                        <span className="material-symbols-outlined text-[#9ca3af] mt-0.5">videocam</span>
                    ...
                    {step === 2 && selectedInterval !== null && intervals[selectedInterval] && (
                        <div className="flex items-start gap-3 mt-4 pt-4 border-t border-white/5">
                            <span className="material-symbols-outlined text-[#9ca3af] mt-0.5">event</span>
